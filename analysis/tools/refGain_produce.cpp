...
//
// Reads EVIO files directly, classifies events by trigger bit and channel count,
...
int main(int argc, char *argv[])
{
    ...
    std::vector<std::string> evio_files =
        CollectInputs(argc, argv, optind, IsEvioName, max_files);
    ...
    std::string ref_gain_dir = prad2::RefGainDir(db_dir);
    {
        std::error_code ec;
    ...
    const auto roc_to_crate = daq_cfg.roc_crate_map();
    ...
    tpl_store.LoadFromConfig(daq_cfg.wave_cfg, db_dir);
        ...
        while (ch.Read() == evc::status::success) {
            ...
            for (int ie = 0; ie < ch.GetNEvents(); ++ie) {
                ...
                for (int r = 0; r < event->nrocs; ++r) {
                    ...
                    fdec::ForEachChannel(roc, [&](int, int, const fdec::ChannelData &) { ++nch; }, 16);
                }

                bool trig_lms      = (event->info.trigger_bits & prad2::TBIT_lms)   != 0;
                bool trig_alpha    = (event->info.trigger_bits & prad2::TBIT_alpha) != 0;

                // Classify by trigger bit and channel count (trigger bits alone are unreliable):
                ...
                for (int r = 0; r < event->nrocs; ++r) {
                    ...
                    fdec::ForEachChannel(roc, [&](int s, int c, const fdec::ChannelData &cd) {
                        ...
                        if (mtyp != prad2::MOD_PbWO4 && mtyp != prad2::MOD_LMS) return;

                        ana.SetChannelKey(roc.tag, s, c);
                        ana.Analyze(cd.samples, cd.nsamples, wres);
                        if (wres.npeaks != 1) return;
                        float integral = wres.peaks[0].integral;
                        if (integral <= 0.f) return;

                        if (mtyp == prad2::MOD_PbWO4) {
                            int wid = mid - kGainWIDBase;   // 1..kGainNW
                            if (wid >= 1 && wid <= kGainNW && is_lms)
                                mod_lms[wid - 1]->Fill(integral);
                        } else {
                            int lid = mid - kGainLMSIDBase; // 0..kGainNLMS-1
                            if (lid >= 0 && lid < kGainNLMS) {
    ...
    // ── Fit reference PMTs ────────────────────────────────────────────────────
    prad2::FitResult fit_lms[kGainNLMS], fit_alpha[kGainNLMS];
    for (int i = 0; i < kGainNLMS; ++i) {
