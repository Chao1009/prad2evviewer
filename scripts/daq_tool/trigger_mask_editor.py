...
Generates trigger mask config text (only slots with a disabled channel
are written).

Usage
-----
    python trigger_mask_editor.py
    python trigger_mask_editor.py -o output.cnf          # default save path
    python trigger_mask_editor.py -i existing.cnf         # load existing mask
    python trigger_mask_editor.py --theme light           # colour theme
"""
...
def _col_on()       -> QColor: return QColor(THEME.SUCCESS)
def _col_off()      -> QColor: return QColor(THEME.DANGER)
...
def generate_trigger_mask(modules: List[Module],
    ...
    for key, ch_map in crate_slots.items():
        mask: List[str] = []
        off_names: List[str] = []
        for ch in range(CHANNELS_PER_SLOT):
            mod_name = ch_map.get(ch)
            if mod_name is None or mod_name in disabled:
                mask.append("0")
                off_names.append(f"ch{ch}:unmapped" if mod_name is None
                                 else mod_name)
            else:
                mask.append("1")
        if off_names:
            blocks[key] = (f"# off: {', '.join(off_names)}", mask)
    return "\n".join(render_fav3(blocks, "FAV3_TRG_MASK"))


def parse_trigger_mask(text: str, modules: List[Module]) -> Set[str]:
    ...
    for crate, slot, vals in iter_fav3(text, "FAV3_TRG_MASK"):
        for ch, val in enumerate(vals):
            mod = daq_lookup.get((crate, slot, ch))
            if val == "0" and mod:
                disabled.add(mod)
    return disabled
...
class HyCalMapWidget(_HyCalMapBase):
    ...
    def __init__(self, modules: List[Module], parent=None):
        super().__init__(parent, shrink=0.92, margin_top=10,
                         ...
                         label_types=AUX_TYPES, show_colorbar=False,
                         min_size=(500, 500))
    ...
    # -- painting: fill-by-state + stats line --
    ...
    # -- mouse: drag-paint replaces the base handlers --
    ...
    def mousePressEvent(self, event):
        ...
        if found:
            ...
            self._paint_mode = found not in self._disabled
...
class TriggerMaskEditor(QMainWindow):
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        btn_enable_all.setStyleSheet(btn_style())
